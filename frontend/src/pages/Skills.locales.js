export default {
  en: {
    title_part1: "Technical",
    title_part2: "Skills",
    desc: "A comprehensive overview of my technical expertise and proficiency levels across different domains.",
    no_skills: "No skills found",
    no_skills_desc: "Skills will appear here once they are added to the backend.",
    certs_title: "Certifications & Awards",
    certs_desc: "Recognized qualifications and achievements in software engineering.",
    cert_1: "AWS Certified Solutions Architect",
    cert_2: "Advanced Python Programming",
    cert_3: "Docker & Kubernetes Masterclass",
    langs_title: "Spoken Languages",
    langs_desc: "Languages I use for daily communication and collaboration.",
    lang_en: "English",
    lang_en_prof: "Professional Working Proficiency",
    lang_nl: "Dutch",
    lang_nl_prof: "Limited Working Proficiency",
    lang_fa: "Persian",
    lang_fa_prof: "Native / Bilingual",
    tools_title: "Tools & Software",
    tools_desc: "Essential instruments in my daily development workflow.",
  },
  nl: {
    title_part1: "Technische",
    title_part2: "Vaardigheden",
    desc: "Een uitgebreid overzicht van mijn technische expertise en vaardigheidsniveaus over verschillende domeinen.",
    no_skills: "Geen vaardigheden gevonden",
    no_skills_desc: "Vaardigheden verschijnen hier zodra ze aan de backend zijn toegevoegd.",
    certs_title: "Certificeringen & Prijzen",
    certs_desc: "Erkende kwalificaties en prestaties in software-engineering.",
    cert_1: "AWS Gecertificeerd Solutions Architect",
    cert_2: "Geavanceerde Python Programmering",
    cert_3: "Docker & Kubernetes Masterclass",
    langs_title: "Gesproken Talen",
    langs_desc: "Talen die ik gebruik voor dagelijkse communicatie en samenwerking.",
    lang_en: "Engels",
    lang_en_prof: "Professionele Werkvaardigheid",
    lang_nl: "Nederlands",
    lang_nl_prof: "Beperkte Werkvaardigheid",
    lang_fa: "Perzisch",
    lang_fa_prof: "Moedertaal / Tweetalig",
    tools_title: "Tools & Software",
    tools_desc: "Essentiële instrumenten in mijn dagelijkse ontwikkelingsworkflow.",
  },
  fa: {
    title_part1: "مهارت‌های",
    title_part2: "فنی",
    desc: "یک مرور جامع از تخصص فنی و سطوح مهارت من در زمینه‌های مختلف.",
    no_skills: "مهارتی یافت نشد",
    no_skills_desc: "مهارت‌ها پس از افزودن در بک‌اند در اینجا نمایش داده خواهند شد.",
    certs_title: "گواهینامه‌ها و افتخارات",
    certs_desc: "مدارک و دستاوردهای معتبر در مهندسی نرم‌افزار.",
    cert_1: "معمار راهکارهای تایید شده AWS",
    cert_2: "برنامه‌نویسی پیشرفته پایتون",
    cert_3: "مسترکلاس داکر و کوبرنتیز",
    langs_title: "زبان‌های گفتاری",
    langs_desc: "زبان‌هایی که برای ارتباط و همکاری روزمره استفاده می‌کنم.",
    lang_en: "انگلیسی",
    lang_en_prof: "تسلط حرفه‌ای در کار",
    lang_nl: "هلندی",
    lang_nl_prof: "تسلط محدود",
    lang_fa: "فارسی",
    lang_fa_prof: "زبان مادری / دوزبانه",
    tools_title: "ابزارها و نرم‌افزارها",
    tools_desc: "ابزارهای ضروری در جریان کاری توسعه روزانه من.",
  },
  de: {
    title_part1: "Technische",
    title_part2: "Fähigkeiten",
    desc: "Ein umfassender Überblick über meine technische Expertise und mein Qualifikationsniveau in verschiedenen Bereichen.",
    no_skills: "Keine Fähigkeiten gefunden",
    no_skills_desc: "Fähigkeiten werden hier angezeigt, sobald sie im Backend hinzugefügt wurden.",
    certs_title: "Zertifizierungen & Auszeichnungen",
    certs_desc: "Anerkannte Qualifikationen und Leistungen im Bereich Software-Engineering.",
    cert_1: "AWS Certified Solutions Architect",
    cert_2: "Fortgeschrittene Python-Programmierung",
    cert_3: "Docker & Kubernetes Masterclass",
    langs_title: "Gesprochene Sprachen",
    langs_desc: "Sprachen, die ich für die tägliche Kommunikation und Zusammenarbeit verwende.",
    lang_en: "Englisch",
    lang_en_prof: "Verhandlungssicher",
    lang_nl: "Niederländisch",
    lang_nl_prof: "Grundkenntnisse",
    lang_fa: "Persisch",
    lang_fa_prof: "Muttersprache / Zweisprachig",
    tools_title: "Werkzeuge & Software",
    tools_desc: "Wesentliche Instrumente in meinem täglichen Entwicklungs-Workflow.",
  }
};
