...
export default function Header() {
 ...
 return (
 <header className="fixed top-0 start-0 z-50 w-full glass">
 ...
 <span className="text-base md:text-xl font-black tracking-tighter">Mahdy</span>
 </Link>
 ...
 {langDropdownOpen && (
 <div className="absolute end-0 mt-2 w-36 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg py-1 z-50">
 {languages.map(lang => (
 <button
 key={lang.code}
 onClick={() => changeLang(lang.code)}
 className={`w-full text-start px-4 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center gap-2 ${i18n.language === lang.code ? 'text-blue-600 font-bold' : 'text-slate-700 dark:text-slate-300'}`}
 >
 <span>{lang.flag}</span>
 ...
 key={link.path} 
 href={link.path} 
 className={`flex items-center gap-3 text-sm md:text-lg font-semibold px-4 py-3 rounded-lg transition-all duration-300 hover:translate-x-2 hover:bg-slate-100 dark:hover:bg-slate-800 ${pathname === link.path ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'text-slate-600 dark:text-slate-300'}`}
