...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full glass">
      ...
      {/* Mobile Nav */}
      {mobileMenuOpen && (
        <div className="md:hidden glass absolute top-16 left-0 w-full border-b border-slate-200 dark:border-slate-800 py-4 px-4 flex flex-col gap-4 shadow-xl animate-slide-up">
          {navLinks.map((link) => (
            <Link 
              key={link.path} 
              to={link.path} 
